...
We found best cp and then prune the tree that gives us trees of various sizes.
We now make prediction on the validation data based on the pruned tree.
...
We fit five different random forests to the training data with different numbers of trees. In this case we take $n =100, 200, 300, 400, 500$. Since we only have 1 variable x, so the dimension of x is 1. This means the mtry = 1.
...
We see that $300$ trees give us lowest root mean squared error on the validation dataset.
We now refit a random forest using $300$ trees on the union of the train and validation data and then later measure the accuracy over the test data.

```{r, echo = FALSE, cache = TRUE}
rffit6 = randomForest(y ~ x, data = cdtrainval, mtry = 1, ntree =300)
rfvalpred6 = predict(rffit6, newdata = cdtest)
rmse6 = sqrt(mean((cdtest$y-rfvalpred6)^2))
print(rmse6)
```
We successfully used the random forest to predict the price of used cars using mileage as the prediction variable.

#### Boosting
We now using Boosting to fit the relate the price and the mileage. We use 8 different  combinations of parameters to fit the models on the test date and chose the one that gives best prediction on the validation data. The eight combinations are the following:
...
plot(p,rmseb)
```
We see that the minimum root mean squared corresponds to the combination 4 which corresponds to the fourth model where we used $5000$ trees with depth 4 and $\alpha = 0.001$.
We now use boosting again on the union of training and validation data and predict on the test data with $5000$ trees with depth $4$ and $\alpha = 0.001$.
...
Use the three set approach, that is, split your data into train, validation, test set.
Plot your results. 
...
#### Different fits with differen size and decay parameters:

Now we fit different single layer neural nets on the training data with size $=$ 25, and 75 and decay $=$ 0.5 and 0.01. We use the size 25 and 75 since the number of the hidden units is usually in the range of 5-100.
```{r, echo = FALSE, cache = TRUE}
...
which.min(nn_rmse)
```
Thus the root mean squared error corresponding to the third fit is minimum of the four which has size = 75 and decay = 0.5.

#### Fit on the union of Train and Validation data:
We now fit a single layer neural net with size = 75 and decay = 0.5 on the train validation set and the predict on the test data.
```{r, echo = FALSE, cache = TRUE}
nn_fit6 = nnet(price ~ mileage, cdtrainval_nn, size = 75, decay = 0.5, linout=T)
...
We again will fit a simple model with various decay rates and sizes using a grid of combinations of both. 
```{r, cache = TRUE}
...
grid1 = cbind(grid, rmse = 0)

for(i in 1:nrow(grid)){
  fit = nnet(price ~ mileage+year, cdtrain_nn, size = grid[i,1], decay = grid[i,2], linout=T, trace = FALSE)
  pred = predict(fit, val)
  grid1[i,3] = rmse(cdval_nn$price,pred)
}
```
For this particular grid search the RMSE is minimized when the grid size is `r grid1[which(grid1$rmse==min(grid1$rmse)),1]` and the the decay is `r grid1[which(grid1$rmse==min(grid1$rmse)),2]`.
...
rmse = rmse(cdtest_nn$price, pred)
```

The RMSE for the test data is `r rmse`. 

#What type of plot make sense for 3 dimeensions like this. 

#If I have time I can use deep learning, but the homework makes me think it's not required. 
