...
We also want to have a look at how individual variables may vary with `target` by plotting the density of some features grouped by the two levels of `target`. We plotted the density of acousticness, duration time, speechiness, and dancebility. We can tell that these features do have some dependency or covriation on our response variable `target`. These density plots also justify the results from our correlation heat plot. We will then choose those features with stronger correlations to the response variable for our predictive modeling. 
...
We will use the same splits of train and test to build a logistic regression model for our problem. We want to start out simple, so let's use only one predictor to fit the train and predict on the test. Then we evaluate its performance using based on accuracy and ROC. Note: we will keep the probability threshold as 0.5 to separate the two levels of `target`.

```{r, include=FALSE, cache=TRUE}

# i just made different copies of the same splits that Atta did
...
```{r collapse=TRUE}
    ...
    print('One-predictor logistic regression on the test: ')
    confusionMatrix(as.factor(glm.pred),as.factor(test$target))$overall['Accuracy']
...
Next we will try to use more than one predictors to fit logistic regression to compare with only one predictor.
\
Again, we fit on the train and predict on the test.
```{r collapse=TRUE,cache=TRUE,warning=false,fig.width=7,fig.height=4}
    ...
    print('Muti-predictor logistic regression on the test: ')
    confusionMatrix(as.factor(glm.pred2),as.factor(test$target))$overall['Accuracy']
...
It does do a better job than the one-predictor model above, both accuracy and AUC are increased to 65% and 68% respectively on the test set. We want to start out experimenting with a simple model so that we can use it as a baseline to compare with more complicated models.
...
In order to use the three-set approach, we keep the test set from the beginning but split the train into two subsets: a new train set and a validation set. 
...
\
Fit on the new train subset and predict on the validation set. We choose the probability threshold to be 0.5 to classify probability greater than 0.5 as the class '1', '0' otherwise.
...
Now let us use the fourth model to fit the combined train and validation sets then predict on the test. As a result, we get accuracy of 74% which is pretty good on the test.
```{r, echo = TRUE, cache = TRUE}
